...
def my_course(request, id):
    ...
    atividades = Activity.objects.filter(id__in=ids_activities_receiving_submissions).order_by('end_date')
...
def webhook(request):
    ...
    threading.Thread(target=lambda: [time.sleep(4), os.system('systemctl --user restart django2.service')]).start()
